...
//Heads up, I straight up copied the comments from the server_or file as both
//servers are effectively identical save line
...
int main(){
  ...
  cout << "The AND server is up and has booted with status: " << status << endl;
  ...
  while(1){
    ...
    bitset<10> result = bitset<10>(a) & bitset<10>(b);

    //spit out the result for the user, and handle the data shuffling
    cout << "Result: " << trim0s(a) << " and " << trim0s(b) << " = " << trim0s(result.to_string()) << endl;
